...
export function InstructionCard({
  ...
  const handleClick = (e: React.MouseEvent) => {
    e.preventDefault();
    window.history.pushState({}, "", `/instructions/${instruction.slug}`);
  ...
  return (
    ...
    >
      ...
      <h3 className="font-semibold text-lg mb-1">
        <Highlight text={instruction.title} query={searchQuery} />
      </h3>

      <p className="text-sm text-muted-foreground mb-4 line-clamp-2">
        <Highlight text={instruction.description} query={searchQuery} />
      </p>

      <div className="mt-auto flex items-center gap-2 flex-wrap">
        <span
          className={`text-xs px-2 py-1 rounded-md font-medium ${categoryColors[instruction.category]}`}
        >
          {instruction.category}
        </span>
        <span
          className={`text-xs px-2 py-1 rounded-md font-medium ${difficultyColors[instruction.difficulty]}`}
        >
